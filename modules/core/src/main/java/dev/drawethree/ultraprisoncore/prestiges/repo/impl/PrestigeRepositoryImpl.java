package dev.drawethree.ultraprisoncore.prestiges.repo.impl;
...
public class PrestigeRepositoryImpl implements PrestigeRepository {
	...
	public long getPlayerPrestige(OfflinePlayer player) {
		try (Connection con = this.database.getConnection(); PreparedStatement statement = con.prepareStatement("SELECT * FROM " + TABLE_NAME + " WHERE " + PRESTIGES_UUID_COLNAME + "=?")) {
	...
	public Map<UUID, Long> getTopPrestiges(int amountOfRecords) {
		...
		try (Connection con = this.database.getConnection(); ResultSet set = con.prepareStatement("SELECT " + PRESTIGES_UUID_COLNAME + "," + PRESTIGES_PRESTIGE_COLNAME + " FROM " + TABLE_NAME + " ORDER BY " + PRESTIGES_PRESTIGE_COLNAME + " DESC LIMIT " + amountOfRecords).executeQuery()) {
			while (set.next()) {
