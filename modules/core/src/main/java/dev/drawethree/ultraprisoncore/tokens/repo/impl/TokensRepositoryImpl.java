...
public class TokensRepositoryImpl implements TokensRepository {
	...
	public long getPlayerTokens(OfflinePlayer p) {
		try (Connection con = this.database.getConnection(); PreparedStatement statement = con.prepareStatement("SELECT * FROM " + TABLE_NAME_TOKENS + " WHERE " + TOKENS_UUID_COLNAME + "=?")) {
	...
	public Map<UUID, Long> getTopTokens(int amountOfRecords) {
		...
		try (Connection con = this.database.getConnection(); ResultSet set = con.prepareStatement("SELECT " + TOKENS_UUID_COLNAME + "," + TOKENS_TOKENS_COLNAME + " FROM " + TABLE_NAME_TOKENS + " ORDER BY " + TOKENS_TOKENS_COLNAME + " DESC LIMIT " + amountOfRecords).executeQuery()) {
			while (set.next()) {
