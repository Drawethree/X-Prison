package dev.drawethree.ultraprisoncore.tokens.repo.impl;
...
public class BlocksRepositoryImpl implements BlocksRepository {
	...
	public long getPlayerBrokenBlocks(OfflinePlayer player) {
		try (Connection con = this.database.getConnection(); PreparedStatement statement = con.prepareStatement("SELECT * FROM " + TABLE_NAME_BLOCKS + " WHERE " + BLOCKS_UUID_COLNAME + "=?")) {
	...
	public long getPlayerBrokenBlocksWeekly(OfflinePlayer player) {
		try (Connection con = this.database.getConnection(); PreparedStatement statement = con.prepareStatement("SELECT * FROM " + TABLE_NAME_BLOCKS_WEEKLY + " WHERE " + BLOCKS_UUID_COLNAME + "=?")) {
			statement.setString(1, player.getUniqueId().toString());
			try (ResultSet set = statement.executeQuery()) {
				if (set.next()) {
					return set.getLong(BLOCKS_BLOCKS_COLNAME);
	...
	public Map<UUID, Long> getTopBlocksWeekly(int amountOfRecords) {
		...
		try (Connection con = this.database.getConnection(); ResultSet set = con.prepareStatement("SELECT " + BLOCKS_UUID_COLNAME + "," + BLOCKS_BLOCKS_COLNAME + " FROM " + TABLE_NAME_BLOCKS_WEEKLY + " ORDER BY " + BLOCKS_BLOCKS_COLNAME + " DESC LIMIT " + amountOfRecords).executeQuery()) {
			while (set.next()) {
	...
	public Map<UUID, Long> getTopBlocks(int amountOfRecords) {
		...
		try (Connection con = this.database.getConnection(); ResultSet set = con.prepareStatement("SELECT " + BLOCKS_UUID_COLNAME + "," + BLOCKS_BLOCKS_COLNAME + " FROM " + TABLE_NAME_BLOCKS + " ORDER BY " + BLOCKS_BLOCKS_COLNAME + " DESC LIMIT " + amountOfRecords).executeQuery()) {
			while (set.next()) {
