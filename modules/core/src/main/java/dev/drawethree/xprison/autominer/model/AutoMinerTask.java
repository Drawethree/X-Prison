...
public final class AutoMinerTask implements Runnable {
	...
	private void decrementPlayerAutoMinerTimeAndNotify(Player p) {
		...
		this.region.getPlugin().getCore().getNmsProvider().sendActionBar(p, this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_enabled"));
	}

	private boolean executeTaskValidationAndNotifyPlayerOnFail(Player p) {

		if (!this.checkPlayerAutoMinerTime(p)) {
			this.region.getPlugin().getCore().getNmsProvider().sendActionBar(p, this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_disabled"));
			return false;
		}

		if (!this.checkPlayerItemInHand(p)) {
			this.region.getPlugin().getCore().getNmsProvider().sendActionBar(p, this.region.getPlugin().getAutoMinerConfig().getMessage("auto_miner_no_pickaxe"));
			return false;
		}
