...
public final class WildPrisonAutoMiner extends ExtendedJavaPlugin {
	...
	private void registerCommands() {
		Commands.create()
				...
				.handler(c -> {
					if (c.args().size() == 0) {
						c.sender().sendMessage(messages.get("auto_miner_time").replace("%time%", String.valueOf(autoMinerTimes.get(c.sender().getUniqueId()))));
					}
	...
	public int getAutoMinerTimeAndDecrement(Player p) {
		if (!hasAutoMinerTime(p)) {
			return 0;
		}
		autoMinerTimes.put(p.getUniqueId(), autoMinerTimes.get(p.getUniqueId()) - 1);
		return autoMinerTimes.get(p.getUniqueId());
	}
