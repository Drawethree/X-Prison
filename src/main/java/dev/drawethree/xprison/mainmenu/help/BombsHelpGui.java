...
public class BombsHelpGui extends Gui {
	...
	public BombsHelpGui(Player player) {
		super(player, 5, "Bombs Help");
	}
	...
	public void redraw() {
		...
		if (isFirstDraw()) {
			...
			this.setItem(13, ItemStackBuilder.of(SkullUtils.INFO_SKULL.clone()).name("&eWhat it is ?").lore(
					"&7Give your players",
					"&7bombs that they can drop (throw)",
					"&7and cause explosions in mine",
					"&7to mine faster."
			).buildItem().build());

			//Commands
			this.setItem(22, ItemStackBuilder.of(SkullUtils.COMMAND_BLOCK_SKULL.clone()).name("&eCommands").lore(
					"&f/bombs give [player] [type] [amount]",
					"&7Gives a player specific bomb."
			).buildItem().build());
