...
public class PlayerHistoryGUI extends Gui {
	...
	public void redraw() {
		...
		//Filter
		this.setItem(20, ItemStackBuilder.of(XMaterial.HOPPER.parseItem()).name("&eFilter History").lore("&7Click to select a filter").build(() -> {
			this.close();
			new PlayerHistoryFilterGUI(this.getPlayer(), this.target, this.plugin).open();
		}));

		//Full History
		this.setItem(22, ItemStackBuilder.of(XMaterial.BOOK.parseItem()).name("&eFull History").lore("&7Click to show full history").build(() -> {
			this.close();
			this.plugin.getHistoryManager().openPlayerHistoryGui(this.getPlayer(), this.target, null);
		}));

		//Clear History
		this.setItem(24, ItemStackBuilder.of(XMaterial.BARRIER.parseItem()).name("&eClear History").lore("&7Click to clear history").build(() -> {
			this.close();
