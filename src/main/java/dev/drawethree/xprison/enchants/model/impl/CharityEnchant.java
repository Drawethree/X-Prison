...
public final class CharityEnchant extends XPrisonEnchantmentBaseCore implements BlockBreakEnchant, ChanceBasedEnchant {

    private double chance;
    private String amountToGiveExpression;
    private boolean messagesEnabled;

    public CharityEnchant() {
    }
    ...
    public void onBlockBreak(BlockBreakEvent e, int enchantLevel) {
        ...
        for (Player p : Players.all()) {
            ...
            if (p.equals(e.getPlayer())) {
                PlayerUtils.sendMessage(p, getEnchants().getEnchantsConfig().getMessage("charity_your").replace("%amount%", String.format("%,d", amount)));
            } else {
                PlayerUtils.sendMessage(p, getEnchants().getEnchantsConfig().getMessage("charity_other").replace("%amount%", String.format("%,d", amount)).replace("%player%", e.getPlayer().getName()));
            }
        }
