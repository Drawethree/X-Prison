...
public class EnchantsManager {
    ...
    public void buyEnchnant(XPrisonEnchantment enchantment, EnchantGUI gui, int currentLevel, int addition) {
        ...
        plugin.getCore().debug(String.format("Calculation of levels %,d - %,d of %s enchant took %dms", currentLevel + 1, currentLevel + addition + 1, enchantment.getRawName(), Time.nowMillis() - startTime), plugin);

        Player player = gui.getPlayer();
        CurrencyType currency = enchantment.getCurrencyType();

        boolean hasEnough = hasEnoughToPurchase(currency,player,totalCost);

        if (!hasEnough) {
            PlayerUtils.sendMessage(player, plugin.getEnchantsConfig().getMessage("not_enough").replace("%currency%", EconomyUtils.getCurrencyName(currency)));
            return;
        }
        ...
        this.setEnchantLevel(player, gui.getPickAxe(), enchantment, currentLevel + addition);

        if (enchantment instanceof EquipabbleEnchantment) {
            ((EquipabbleEnchantment) enchantment).onUnequip(player, gui.getPickAxe(), currentLevel);
            ((EquipabbleEnchantment) enchantment).onEquip(player, gui.getPickAxe(), currentLevel + addition);
        }

        player.getInventory().setItem(gui.getPickaxePlayerInventorySlot(), gui.getPickAxe());

        String formattedCost = String.format("%,d", totalCost);

        if (addition == 1) {
            PlayerUtils.sendMessage(player, plugin.getEnchantsConfig().getMessage("enchant_bought")
                    .replace("%currency%", EconomyUtils.getCurrencyName(currency))
                    .replace("%amount%", formattedCost));
        } else {
            PlayerUtils.sendMessage(player, plugin.getEnchantsConfig().getMessage("enchant_bought_multiple")
                    .replace("%currency%", EconomyUtils.getCurrencyName(currency))
    ...
    public void disenchant(XPrisonEnchantment enchantment, DisenchantGUI gui, int currentLevel, int substraction) {
        ...
        switch (currency) {
            ...
            case VAULT:
                ...
                break;
            default:
                PlayerUtils.sendMessage(player, "&cInvalid currency type for disenchant refund: " + currency);
                return;
        }

        this.setEnchantLevel(player, gui.getPickAxe(), enchantment, currentLevel - substraction);

        if (enchantment instanceof EquipabbleEnchantment) {
            ((EquipabbleEnchantment) enchantment).onUnequip(player, gui.getPickAxe(), currentLevel);
            ((EquipabbleEnchantment) enchantment).onEquip(player, gui.getPickAxe(), currentLevel - substraction);
        }

        player.getInventory().setItem(gui.getPickaxePlayerInventorySlot(), gui.getPickAxe());

        String formattedRefund = String.format("%,d", totalRefunded);
        String formattedLevels = String.valueOf(substraction);

        PlayerUtils.sendMessage(player,
                plugin.getEnchantsConfig().getMessage("enchant_refunded")
                        .replace("%amount%", formattedLevels)
                        .replace("%enchant%", enchantment.getName()));

        PlayerUtils.sendMessage(player,
                plugin.getEnchantsConfig().getMessage("enchant_refund_amount")
                        .replace("%amount%", formattedRefund)
                        .replace("%currency%", EconomyUtils.getCurrencyName(currency)));
    }
    ...
    public void disenchantMax(XPrisonEnchantment enchantment, DisenchantGUI gui, int currentLevel) {
        ...
        Schedulers.async().run(() -> {
            ...
            PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_refund_amount").replace("%currency%", EconomyUtils.getCurrencyName(currency)).replace("%amount%", String.format("%,d", totalRefunded)));
        });
    }
    ...
    public void buyMaxEnchant(XPrisonEnchantment enchantment, EnchantGUI gui, int currentLevel) {
        ...
        Schedulers.async().run(() -> {
            ...
            while (gui.getPlayer().isOnline() && (currentLevel + levelsToBuy + 1) <= enchantment.getMaxLevel() && hasEnoughToPurchase(currency, gui.getPlayer(), totalCost + EnchantUtils.getCostOfLevel(enchantment, currentLevel + levelsToBuy + 1))) {
                levelsToBuy += 1;
            ...
            if (levelsToBuy == 0) {
                this.lockedPlayers.remove(gui.getPlayer().getUniqueId());
                PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("not_enough").replace("%currency%", EconomyUtils.getCurrencyName(currency)));
                return;
            }
            ...
            if (levelsToBuy == 1) {
                PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_bought").replace("%currency%", EconomyUtils.getCurrencyName(currency)).replace("%amount%", String.format("%,d", totalCost)));
            } else {
                PlayerUtils.sendMessage(gui.getPlayer(), plugin.getEnchantsConfig().getMessage("enchant_bought_multiple")
                        ...
                        .replace("%enchant%", enchantment.getName())
                        .replace("%amount%", String.format("%,d", totalCost)));
            }
        });
    }
