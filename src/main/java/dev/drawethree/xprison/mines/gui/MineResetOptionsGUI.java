...
public class MineResetOptionsGUI extends Gui {
	...
	public void redraw() {
		...
		this.setItem(13, ItemStackBuilder.of(XMaterial.COMPARATOR.parseItem()).name("&eReset Type: " + this.mine.getResetType().getName()).lore(" ", "&7Instant: Will use more CPU power", "&7but the mine will reset instantly.", " ", "&7Gradual: Will use less CPU power", "&7but mine reset may take more time.", " ", "&aClick &7to change.").build(() -> {
		...
		this.setItem(31, ItemStackBuilder.of(XMaterial.PAPER.parseItem()).name("&eBroadcast Reset: " + this.mine.isBroadcastReset()).lore(" ", "&aTrue &7- All players will get message", "&7on mine's reset.", "&cFalse &7- No broadcast message.").build(() -> {
