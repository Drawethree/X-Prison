...
public class RankManager {
	...
	private void loadPrestiges() {
		...
		this.prestigeCost = this.plugin.getConfig().get().getDouble("prestige_cost");
		this.prestigeById = new LinkedHashMap<>();

		for (int i = 0; i <= this.plugin.getConfig().get().getInt("max_prestige"); i++) {
			String prefix = Text.colorize(this.plugin.getConfig().get().getString("prestige_prefix").replace("%prestige%", String.valueOf(i)));
			Prestige p = new Prestige(i, prestigeCost, prefix, new ArrayList<>());
	...
	public boolean buyNextRank(Player p) {
		...
		if (!this.plugin.getCore().getEconomy().has(p, toBuy.getCost())) {
			p.sendMessage(this.plugin.getMessage("not_enough_money").replace("%cost%", String.format("%f", toBuy.getCost())));
			return false;
		}
	...
	public boolean buyNextPrestige(Player p) {
		...
		if (!this.plugin.getCore().getEconomy().has(p, toBuy.getCost())) {
			p.sendMessage(this.plugin.getMessage("not_enough_money_prestige").replace("%cost%", String.format("%f", toBuy.getCost())));
			return false;
		}
