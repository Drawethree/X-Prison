...
public class LevelCommandAutoMinerGui extends Gui {
	...
	private static final ItemStack LOCKED_NEXT_ITEM = ItemStackBuilder.of(Material.valueOf(WildPrisonAutoMiner.getInstance().getConfig().get().getString("level_up_misc_gui.locked_next_item.material")))
	...
	@Override
	public void redraw() {
		int playerCommandLevel = WildPrisonAutoMiner.getInstance().getPlayerCommandLevel(this.getPlayer());
	...
	private ItemStack getNextLevelItem() {
		...
		for (int i = 0; i < lore.size(); i++) {
			lore.set(i, lore.get(i).replace("%cost%", String.format("%,d", WildPrisonAutoMiner.getInstance().getNextCommandLevel(this.getPlayer()).getCost())));
		}
		meta.setLore(lore);
